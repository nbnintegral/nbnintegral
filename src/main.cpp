...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2014-04-19 Adriano https://nbnintegraltalk.org/index.php?action=profile;u=112568
        // The following address was lost during distribution with 196780608.602771 coins in it. Blocking just in case :-)
        static const CnbnintegralAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){  // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){  // extract the destination of the previous transaction's vout[n]
                CnbnintegralAddress addressSource(source);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal nbnintegral miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in nbnintegral
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("nbnintegralMiner : proof-of-work not meeting target");

    //// debug print
    printf("nbnintegralMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("nbnintegralMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("nbnintegralMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadnbnintegralMiner(void* parg);

static bool fGeneratenbnintegrals = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void nbnintegralMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("nbnintegral-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratenbnintegrals || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratenbnintegrals) && !fProofOfStake)
                return;
        }
        ...
        printf("Running nbnintegralMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadnbnintegralMiner(void* parg)
{
    ...
    try
    {
        ...
        nbnintegralMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadnbnintegralMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadnbnintegralMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadnbnintegralMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatenbnintegrals(bool fGenerate, CWallet* pwallet)
{
    fGeneratenbnintegrals = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratenbnintegrals = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d nbnintegralMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadnbnintegralMiner, pwallet))
                printf("Error: NewThread(ThreadnbnintegralMiner) failed\n");
            Sleep(10);
        }
    }
}
