    ...
    model(0)
{
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::nbnintegralAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(nbnintegralUnits::formatWithUnit(nbnintegralUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
...
//    WalletModel::SendCoinsReturn sendstatus = model->sendCoins(recipients);
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(nbnintegralUnits::formatWithUnit(nbnintegralUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parsenbnintegralURI(uri, &rv))
    {
        CnbnintegralAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    ui->labelBalance->setText(nbnintegralUnits::formatWithUnit(unit, balance));
}
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(nbnintegralUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
// Coin Control: copy label "Quantity" to clipboard
 ...
 void SendCoinsDialog::coinControlChangeChecked(int state)
 {
     ...
     {
         if (state == Qt::Checked)
             CoinControlDialog::coinControl->destChange = CnbnintegralAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
         else
 ...
 void SendCoinsDialog::coinControlChangeEdited(const QString & text)
 {
     if (model)
     {
         CoinControlDialog::coinControl->destChange = CnbnintegralAddress(text.toStdString()).Get();

         // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
             ui->labelCoinControlChangeLabel->setText("");
         else if (!CnbnintegralAddress(text.toStdString()).IsValid())
         {
         ...
         else
         {
             ...
             else
             {
                 CPubKey pubkey;
                 CKeyID keyid;
                 CnbnintegralAddress(text.toStdString()).GetKeyID(keyid);
